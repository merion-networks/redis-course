...
@app.get("/list/length")
async def get_list_length(key: str):
    """
    Возвращает длину списка, хранящегося в Redis по ключу 'key'.
    Использует команду LLEN.

    Пример запроса:
      GET /list/length?key=tasks
    """
...
async def get_union(key1: str, key2: str):
    ...
    """
    union_values = await redis_client.sunion(key1, key2)
...
async def get_difference(key1: str, key2: str):
    ...
    """
    diff_values = await redis_client.sdiff(key1, key2)
