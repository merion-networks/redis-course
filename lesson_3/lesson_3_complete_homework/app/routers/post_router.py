...
# Зависимость для получения обязательного аутентифицированного пользователя
...
# Зависимость для получения аутентифицированного пользователя (опционально)
...
async def read_post(
    ...
    # Если пользовватель аутентифицирован сохраняем id поста в его истоприи просмотров
    if current_user:
...
async def like_post(
    ...
    key = f"likes:{post_id}"
    await redis_client.redis.sadd(key, current_user["user_id"])
...
# --- Эндпоинт для получения истории просмотров текущего пользователя ---
