...
async def read_post(
    ...
    if cached_post:
        ...
        await redis_client.set(cache_key, json.dumps(post_data), ex=360)
...
async def create_post(
    ...
    """
    Создаёт новый пост. Автором становится текущий аутентифицированный пользователь.
    После сохранения поста в базу, публикуется уведомление для подписчиков автора.
    """
    user_id = int(current_user["user_id"])
    new_post = Post(
        name=post.name,
    ...
    # Преобразуем данные поста в словарь для уведомления
    post_data = PostSchema.model_validate(new_post).model_dump()
    
    # Публикуем уведомление в канал "new_post:{author_id}"
    channel = f"new_post:{new_post.author_id}"
    await redis_client.redis.publish(channel, json.dumps(post_data))
